...
'''
    ...
    def apiExecutionOutput = {
        if (!apiService.requireVersion(request, response, ApiRequestFiltersUtil.V5)) {
            return
        }
        ...
        def dataClos= {
            outputData.each {
                ...
                if (outf == 'json') {
                    ...
                    //xml
                    if (apiVersion <= ApiRequestFiltersUtil.V5) {
    ...
    def apiExecutionStateOutput = {
        if (!apiService.requireVersion(request, response, ApiRequestFiltersUtil.V10)) {
            return
        }
...
//        }
    ...
    * API actions
    ...
    def apiExecutionAbort(){
        ...
        def killas=null
        if (params.asUser && apiService.requireVersion(request, response, ApiRequestFiltersUtil.V5)) {
    ...
    def apiExecutionDelete (){
        if (!apiService.requireVersion(request, response, ApiRequestFiltersUtil.V12)) {
            return
        }
        def Execution e = Execution.get(params.id)
        if(!apiService.requireExists(response,e,['Execution ID',params.id])){
