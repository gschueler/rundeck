...
class ScheduledExecutionController  extends ControllerBase{
    ...
    */
    ...
    def apiFlipExecutionEnabledBulk(ApiBulkJobDeleteRequest deleteRequest) {
        if(!apiService.requireVersion(request, response, ApiRequestFiltersUtil.V16)){
            return
        }
        if (deleteRequest.hasErrors()) {
    ...
    */
    def apiJobDeleteBulk(ApiBulkJobDeleteRequest deleteRequest) {
        ...
        if (request.api_version < ApiRequestFiltersUtil.V14 && !(response.format in ['all', 'xml'])) {
...
//            }
    ...
    private def runAdhoc(ApiRunAdhocRequest runAdhocRequest){
        ...
        if (runAdhocRequest.asUser && apiService.requireVersion(request, response, ApiRequestFiltersUtil.V5)) {
    ...
    * API Actions
    ...
    def apiJobsImport(){
        ...
        def parseresult
        if(request.api_version >= ApiRequestFiltersUtil.V14 && request.format=='xml'){
            //xml input
            parseresult = scheduledExecutionService.parseUploadedFile(request.getInputStream(), 'xml')
        }else if(request.api_version >= ApiRequestFiltersUtil.V14 && request.format=='yaml'){
            //yaml input
        ...
        def jobset = parseresult.jobset
        if(request.api_version >= ApiRequestFiltersUtil.V14){
            //require project parameter
            if(!apiService.requireParameters(params,response, ['project'])){
                return
            }
        }
        if(request.api_version >= ApiRequestFiltersUtil.V8){
        ...
        if (request.api_version < ApiRequestFiltersUtil.V9) {
            option = null
        }
    ...
    def apiJobRun() {
        ...
        if (request.method == 'GET' && request.api_version >= ApiRequestFiltersUtil.V14) {
...
//                    )
        ...
        if(jobAsUser && apiService.requireVersion(request, response, ApiRequestFiltersUtil.V5)) {
            // authorize RunAs User
        ...
        if (request.api_version >= ApiRequestFiltersUtil.V18 && jobOptions && jobOptions instanceof Map) {
            jobOptions.each { k, v ->
        ...
        def result
        if (request.api_version > ApiRequestFiltersUtil.V17 && jobRunAtTime) {
...
//        }
    ...
    def apiRunScriptUrl (ApiRunAdhocRequest runAdhocRequest){
        ...
        if (!apiService.requireVersion(request, response, ApiRequestFiltersUtil.V4)) {
            return
        }
    ...
    private def apiJobExecutionsResult(boolean apiRequest) {
        ...
        if (apiRequest && request.api_version < ApiRequestFiltersUtil.V14 && !(response.format in ['all', 'xml'])) {
            return apiService.renderErrorFormat(response,[
                    status:HttpServletResponse.SC_UNSUPPORTED_MEDIA_TYPE,
                    code: 'api.error.item.unsupported-format',
                    args: [response.format]
    ...
    def apiJobClusterTakeoverSchedule (){
        ...
        def api17 = request.api_version >= ApiRequestFiltersUtil.V17
