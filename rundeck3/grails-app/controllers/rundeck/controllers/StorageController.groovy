...
class StorageController extends ControllerBase{
    ...
    private def pathUrl(path){
        def uriString = "/api/${ApiRequestFiltersUtil.API_CURRENT_VERSION}/incubator/storage/$path"
        if ("${path}".startsWith('keys/') || path.toString() == 'keys') {
            uriString = "/api/${ApiRequestFiltersUtil.API_CURRENT_VERSION}/storage/$path"
        }
