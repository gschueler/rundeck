...
public class URLFileUpdater implements FileUpdater {
    ...
    private void updateFileUrl(final File destinationFile) throws FileUpdaterException {
        try {
            ...
            contentType = srfile.getName().endsWith(".xml") ? "text/xml" : srfile.getName().endsWith(".yaml")
                                                                           || srfile.getName().endsWith(".yml")
                                                                           ? "text/yaml" : null;
    ...
    private void cacheResponseInfo(final HttpMethod method, final File cacheFile) {
        ...
        if (newprops.size() > 0) {
            try {
                newprops.store(new FileOutputStream(cacheFile), "URLNodesProvider cache data");
            } catch (IOException e) {
                logger.debug(
